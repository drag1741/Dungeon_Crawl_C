...
void move_character_imp(struct Character *character, const int input, const struct Floor *floor){
    if(input == KEY_LEFT || input == 'h'){
        if(character->allowed_to_move(character,floor->graph[character->y_position][character->x_position-1]))
            character->x_position -= 1;
    }
    else if(input == KEY_UP || input == 'k'){
        if(character->allowed_to_move(character,floor->graph[character->y_position-1][character->x_position]))
